...
BpfBytecode CodegenLLVM::emit()
{
  ...
  const auto &imports = ((ast::Program*)root_)->imports;
  if (imports.empty())
  ...
  if (bpf_linker__add_fd(linker, origFd, nullptr)) {
  ...
  for (const auto *import : imports) {
    ...
    // TODO close FDs?
    int libFd = open(import->path().c_str(), O_RDONLY);
    if (bpf_linker__add_fd(linker, libFd, nullptr)) {
