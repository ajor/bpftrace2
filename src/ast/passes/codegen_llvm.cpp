...
BpfBytecode CodegenLLVM::emit()
{
  ...
  struct bpf_linker *linker = bpf_linker__new_fd("memfd:linked_output", newfd, nullptr);
  if (!linker) {
    // TODO error here
  }

  if (bpf_linker__add_buf(linker, "bpftrace_bytecode", output.data(), output.size(), nullptr)) {
