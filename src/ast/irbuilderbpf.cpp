...
// It represents the inode of the init PID namespace, i.e. the root namespace
...
Value *IRBuilderBPF::CreateGetPid(Value *ctx, const location &loc)
{
  ...
  if (pidns.st_ino != PROC_PID_INIT_INO)
  {
    ...
    Value *pid = CreateLoad(getInt32Ty(), CreateGEP(BpfPidnsInfoType(), res, { getInt32(0), getInt32(0) }));
...
Value *IRBuilderBPF::CreateGetTid(Value *ctx, const location &loc)
{
  ...
  if (pidns.st_ino != PROC_PID_INIT_INO)
  {
    ...
    CreateGetNsPidTgid(
        ctx, getInt64(pidns.st_dev), getInt64(pidns.st_ino), res, loc);
    Value *tid = CreateLoad(getInt32Ty(), CreateGEP(BpfPidnsInfoType(), res, { getInt32(0), getInt32(1) }));
    CreateLifetimeEnd(res);
    return tid;
  }
