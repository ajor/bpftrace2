...
public:
  ...
  Import(std::string name, std::filesystem::path path, std::optional<std::filesystem::path> user_path)
    : name_(std::move(name)), path_(std::move(path)), user_path_(user_path) {}

  const std::string &name() const { return name_; }
  const std::filesystem::path &path() const { return path_; }
  const std::optional<std::filesystem::path> &user_path() const { return user_path_; }
