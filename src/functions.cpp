                                ...
                                const std::vector<Param> &params)
{
  ...
  for (const Function &func : funcs_by_fq_name_[fq_name]) {
    ...
    if (is_dup) {
      throw std::runtime_error("Cannot redefine existing function: " + fq_name.str() +
 ...
 * 2. If there are multiple candidates and one is an exact type match, pick that
